...
import { Download, Upload, FileSpreadsheet, CheckCircle, Circle, Search, ChevronLeft, ChevronRight, BarChart2, LogOut, Calculator, Trash2 } from 'lucide-react';
...
export default function Home() {
  ...
  const [categoryFilter, setCategoryFilter] = useState<'all'|'salary'|'allowance'>('all');
  ...
  const deleteRecord = async (e: React.MouseEvent, recordId: string) => {
    e.stopPropagation();
    if (!confirm('คุณแน่ใจหรือไม่ว่าต้องการลบบิลนี้? (หากลบแล้วจะไม่สามารถกู้คืนได้)')) return;

    // Optimistic UI update
    setRecords(prev => prev.filter(r => r.id !== recordId));

    try {
      const res = await fetch(`/api/payments?recordId=${encodeURIComponent(recordId)}`, {
        method: 'DELETE'
      });
      if (!res.ok) throw new Error('Failed');
  ...
  const executeDeleteBatch = async () => {
    ...
    try {
      const res = await fetch(`/api/payments?batchPrefix=${encodeURIComponent(batchPrefix)}`, {
        method: 'DELETE'
      });
      if (!res.ok) throw new Error('Failed');
  ...
  const batches = useMemo(() => {
    const map = new Map<string, { prefix: string, count: number, totalAmount: number, isAllowance: boolean, title: string }>();
    ...
    records.forEach(r => {
      ...
      if (parts.length >= 4) {
        ...
        if (prefix) {
          if (!map.has(prefix)) {
            map.set(prefix, { prefix, count: 0, totalAmount: 0, isAllowance, title });
          }
  ...
  const filteredRecords = useMemo(() => {
    const filtered = displayRecords.filter(r => {
      ...
      const matchCategory = categoryFilter === 'all' || r.paymentType === categoryFilter;
  ...
  const paidRecords = filteredRecords.filter(r => r.isPaid).length;
              ...
              <div key={b.prefix} className="card flex-between" style={{ padding: '20px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                    จำนวน {b.count} รายการ • ยอดรวม ฿{b.totalAmount.toLocaleString()}
                  </div>
                </div>
                <button
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    ยอดตู้: <strong style={{ color: 'var(--text-primary)', fontSize: '1.05rem' }}>฿{(record.payableAmount !== undefined ? record.payableAmount : record.amount).toLocaleString()}</strong>
                    {(record as any).otherDeductions > 0 && (
        ...
        <div style={{
          ...
          <div className="card animate-fade-in" style={{
            ...
            <div style={{ fontSize: '0.95rem', color: 'var(--text-secondary)', marginBottom: '32px', lineHeight: '1.6' }}>
              ข้อมูลบิลทั้งหมดในรอบนี้จะถูกลบออกจากระบบอย่างถาวร (คุณอาจต้องตรวจสอบยอดทบสะสมอีกครั้งหากมีการเปลี่ยนแปลง)
            </div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button 
                ...
                style={{ flex: 1, padding: '12px', borderRadius: '12px', background: 'var(--text-primary)', border: 'none', color: 'white', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}
              >
                ยืนยันการลบ
              </button>
            </div>
